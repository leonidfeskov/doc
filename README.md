...
* [Соглашение по именованию CSS-классов](#Соглашение-по-именованию-css-классов)
	* [CSS-селекторы](#CSS-селекторы)
	* [Правила формирования имен](#Правила-формирования-имен)
		* [Имя блока](#Имя-блока)
		* [Имя элемента](#Имя-элемента)
		* [Имя модификатора](#Имя-модификатора)
	* [Пример использования](#Пример-использования)
* [Система организации CSS](#Система-организации-css)
	* [Слой foundation](#Слой-foundation)
	* [Слой base](#Слой-base)
	* [Слой project](#Слой-project)
	* [Слой cosmetic](#Слой-cosmetic)
	* [Слой context](#Слой-context)
* [Примеры работы над проектом](#Примеры-работы-над-проектом)
	* [Добавление нового блока](#Добавление-нового-блока)
	* [Подключение нового js-плагина](#Подключение-нового-js-плагина)
	* [Изображения](#Изображения)

# Начало работы

1.  Склонируйте репозиторий
2.  Установите [node.js](https://nodejs.org/) и [bower](http://bower.io/)
3.  Установите gulp `npm install gulp -g`
4.  Перейдите в папку с проектом и выполните команду `npm install`. Дождитесь установики npm-модулей.
5.  Выполните команду `bower install`. Дождитесь установики bower-компонентов.
6.  Выполните команду `gulp build` для первой сборки проекта. Запустите команду `gulp` для начала работы с проектом.

### Другие команды gulp
* `gulp clean` - очищает папку htdocs
* `gulp sprites` - перегенерация спрайтов
* `gulp mts-common` - собирает одельно шапку и подвал для подлючения на других сайтах.

### Возможные ошибки компиляции
* Если сборщик ругается на спрайты и новое изображение не добавилось в спрайт, остановить gulp, запустите отдельно `gulp sprites`, затем заново пересоберите проект `gulp build`
* Если сборщик ругается на javascript, возможно он не скопировал js-файлы в папку temp. Запустите отдельно команду `gulp scripts-copy`
...
# Структура проекта
Структура проекта содержит в себе два основных каталога: source и htdocs.
Разработчик взаимодействует только с каталогом source. Содержимое каталога htdocs генерирует сборщик статики.
...
Блоки в папке blocks лежат структурировано, согласно [методологии разделения стилей по слоям](#Система-организации-css): foundation, base, project, cosmetic.  
...
Каталог htdocs содержит скомпилированные html-страницы и статику.
```
htdocs/
    f/
        css/
...
3.  В созданную папке блока добавьте следующие файлы:  
    ...
    * каталог `i/sprite/` - содержит изображения, которые попадут в общий **png**-спрайт `htdocs/f/i/sprite-icons.png`;
    * каталог `i/sprite@2x/` - содержит изображения, которые попадут в общий **png**-спрайт `htdocs/f/i/sprite-icons@2x.png`;
...
1. Найдите нужный плагин в bower.
2. Из корневого каталога проекта выполните команду `bower instal PLUGIN-NAME --save`. Дождитесь установки плагина.
3. Пропишите плагин в package.json. 
```json
"browser": {
	...
	"jScrollPane": "./bower_components/jScrollPane/script/jquery.jscrollpane.min.js",
	...
	},
"browserify-shim": {
	...
...
```
4. После этого можете использовать плагин, подключив его в вашем js-файле 
```javascript
	require('jScrollPane');
```
...
...
Положите svg-изображение в папку source/blocks/base/b-icon/i/sprite-svg/. После добавления нового изображения в спрайт необходимо его перегенерировать. Для этого перезапустите команду `gulp`. Затем можете использовать svg-изображение в jade из спрайта:
```html
svg
    use(xlink:href="#b-icon_internet")
```
Сам спрайт вставлен в html-документ как `<svg>`.
